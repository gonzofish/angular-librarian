...
const getKnownSelector = (providedSelector = '') => {
    let { pkg, selector } = utilities.getPackageSelector(providedSelector);
    let questions = [];
    let answers = [];
    ...
    if (utilities.checkIsDashFormat(selector)) {
        ...
        questions = questions.concat(getSelectorQuestions);
    }
...
const getTemplates = (rootDir) => {
    const componentDir = (file) => src('{{ package }}', 'src', '{{ selector }}', file);
...
const notifyUser = (answers) => {
    ...
    const selector = answers.find((answer) => answer.name === 'selector').answer;

    console.info(`\nDon't forget to add the following to the ${ pkg }/src/${ pkg }.module.ts file:`);
    console.info(`    import { ${componentName} } from './${selector}/${selector}.component';`);
    console.info(`And to add ${componentName} to the NgModule declarations list`);
};
